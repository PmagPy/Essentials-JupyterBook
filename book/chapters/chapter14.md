...
2. *Biospheric interaction:* Some life forms make magnetic crystals (Figure 6.12 in Chapter 6). In the case of magnetotactic bacteria, these tiny magnets are used for physical orientation. In some cases, animals may use magnetic field lines for navigation.
...
a) VDM versus VGP latitude from data in the PINT06 database compiled by {cite:t}`tauxe2007`. The red triangles are from double heating experiments with pTRM checks (see Chapter 10). b) Plot of transitional VGPs (blue dots) from the TRANS data base {cite:p}`mcelhinny1996`. No selection criteria were applied. c) Shear wave velocity SB448 model of {cite:t}`masters2000` evaluated at 2770 km (core mantle boundary region). There is a fast (cold) ring around the Pacific, presumably from the influence of subducted slabs.
:::
...
There has been considerable effort in collecting the data relevant to describing the statistical character of the geomagnetic field over time. Selected results from one such collection (that of {cite:t}`mcelhinny1997`; MM97) are shown in [](#fig:psvrl). Directions from lava flows less than five million years old from particular latitudinal bands are plotted with respect to the expected GAD direction at that particular latitude ($D',I'$ from Chapter 2).

Several things are worth mentioning about the data in [](#fig:psvrl). First, it appears that the equatorial data are more elongate than those from higher latitudes (something we mentioned in Chapter 12). The elongation parameter $E$ can be used to quantify this and is the $\tau_2/\tau_3$ ratio where $\tau_i$ are the eigenvalues of the orientation matrix (see Appendix on eigenparameters). Secondly, the scatter in the directional data seems to go down with increasing latitude. Thirdly, when the directions are converted to VGPs, the dispersion in VGPs tends to increase with latitude.
...
The most recent model of the statistical paleosecular variation genre is the TK03.GAD model of {cite:t}`tauxe2004b`; see also {cite:t}`tauxe2008`. Like CP88.GAD, TK03.GAD has only three parameters: $\bar g_1^0$ (set to fit a recent estimate for the long term average intensity of the axial dipole as in [](#fig:pint)), $\alpha$ as defined in CP88, but fit to the more recent compilation of directional data of {cite:t}`mcelhinny1997` and a new paramter $\beta$ which is the ratio of the asymmetric ($l+m$ odd) to the symmetric ($l+m$ even) gauss coefficients for a given $l$. We show the variation in $\sigma$ with degree for the two families (asymmetric and symmetric) in [](#fig:tk03)a. The term $\beta$ allows a much improved fit to the paleomagnetic observations (see [](#fig:tk03)b) while the model retains the simplicity of the CP88 model. Please note that a new generation of models is on the way that will incorporate the vast amount of new data being generated (for a preview of things to come, see {cite:t}`johnson2008`).
...
Data from the last five million years fit the model predictions as it was designed to do, but the model can be tested through time by calculating the elongation/inclination pair for data sets of any age. The requirements are that the data are referenced to paleo-horizontal, that the directions represent the ancient geomagnetic field (they are not biased by overprinting, inclination error, etc.), and that there be a sufficient number to represent the statistical variability of the ancient geomagnetic field. There are not many data sets that satisfy these requirements. {cite:t}`tauxe2008` compiled data sets from ancient large igneous provinces that did: the Deccan Traps in India {cite:p}`vandamme1991; vandamme1992`, the Faroe Island basalts {cite:p}`riisager2002`, and Kerguelen {cite:p}`plenier2002`. The elongation/inclination pairs from these data sets are plotted on [](#fig:tk03)d for comparison with the model predictions. It appears that the TK03.GAD model can be used as a guide to the geomagnetic field behavior for at least the Tertiary.

**SUPPLEMENTAL READINGS:** {cite:t}`tauxe2008`; {cite:t}`johnson2008`.
...
c) Use the function **pmag.doprinc** to calculate the eigenparameters of the orientation matrix of the Hawaiian directions (see Chapter 9 and the Appendix on Orientation Tensors at: https://earthref.org/MagIC/books/Tauxe/Essentials/WebBook3ap1.html#x20-220000A.3.5). What is wrong with either the GAD model or the data set?
