...
**Figure 5.2:** Variation of energy densities and their derivatives as a function of magnetization angle $\theta$ for a prolate spheroid magnetite particle (aspect ratio $q = 1.5$) with the applied field antiparallel to the initial magnetization ($\phi$ = 0°). The energy terms are: $\epsilon_a$ (anisotropy energy from grain shape), $\epsilon_m$ (magnetostatic/Zeeman energy from the applied field), and $\epsilon_t$ (total energy = $\epsilon_a + \epsilon_m$). Use the slider to increase B from 0-100 mT and observe how the energy landscape evolves. When the flipping condition is met (both derivatives cross zero at the same angle), the magnetization can irreversibly reverse direction. 
